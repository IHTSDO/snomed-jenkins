...
case ${SNOMED_PROJECT_LANGUAGE,,} in
    cypress | typescript | javascript | embedded)
    ...
    *)
		...
		echo "Using NVD API Key starting: \"${NVD_API_KEY:0:5}\""
                mvn -U dependency-check:check -DskipTests -Dnvd.api.key=$NVD_API_KEY -DossIndexAnalyzerEnabled=true -DossIndexUsername=$OSS_USERNAME -DossIndexPassword=$OSS_TOKEN
